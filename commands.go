...
var (
	bashShell            bool
	listWindowsJSON      bool
	listPanesJSON        bool
	displayMessageJSON   bool
	controlMode          bool
	attachTarget         string
	sessionName          string
...
// remote proxy commands (thin wrappers over RunControlCommand)
...
// store inspection / checkpoint commands
...
// remoteCLIFlags describes which flag bindings a remote-proxy command needs.
...
// remoteCLICommand creates a cobra.Command that proxies to the daemon via
// RunControlCommand.  The positional argument check is optional.
...
// remoteCLICommandWithFlags is like remoteCLICommand but rewrites RunE to
// inject -t / -c flags into the forwarded argv.
...
// registerRemoteCLIFlags attaches -t / -c flags to commands that need them
// (because their RunE closure reads the shared vars).
